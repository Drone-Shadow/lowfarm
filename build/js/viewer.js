  ...
  .then(config => {
    ...
    if (config.overlays) {
      for (const key in config.overlays) {
        ...
        overlays[ov.name] = layer;

        // Only add to map if visible = true
        if (ov.visible) {
          initialOverlays.push(layer);
        }
      }
      ...
      initialOverlays.push(ortho);
    }

    // --- Init map with default basemap + only visible overlays ---
    const map = L.map('map', {
      center: config.center ?? [53.525, -0.397],
      zoom: config.zoom ?? 15,
      layers: [defaultBase, ...initialOverlays]
    });
    ...
    function getTopOverlay() {
      ...
      return overlayLayers[overlayLayers.length - 1];
    }
